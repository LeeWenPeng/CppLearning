...
针对数独游戏素材的 Makefile 文件编写

![[数独软件的makefile文件|1000]]

在 Makefile 文件中 code 编写的顺序是依赖顺序，是与实际编译顺序相反的
...
> - 查看 cpp 文件生成.o 文件所依赖文件的命令 `g++ -MM 文件名.cpp`
> - hpp 后缀文件不需要编译
> - 依赖中的头文件可以省略掉，但不建议
> - 如果目标与最终目标没有直接或间接的依赖关系，就不会自动执行，比如上述的 `clean` 目标
> - **如果依赖没有改变，make 工具将不会重新编译和链接**
...
- 可以通过添加关键字 `.ONESHELL`，使得所有的 Shell 命令调用同一个 Shell 进程中执行
...
在执行 Shell 命令时，首先会打印 Shell 命令，然后再打印 Shell 命令的执行结果。
...
通过在命令前添加 `-` 的方式，使得当命令执行错误时，make 工具忽略错误，继续执行之后的 shell 命令。
...
Makefile 中的变量一般是全局变量，也就是说，在变量定义后，可以在 Makefile 文件中任意位置对变量进行引用。
...
> 结果：相当于使用了 `$` 的转义字符，终端处报错，错误原因为不存在该 var
...
#### 19.1.2  使用 VPATH 修改 Makefile 文件
...
### 19.2  通过 vpath 指令指定搜索路径

vpath 用法比 VPATH 使用更加灵活，可以为 make 指定文件类型的搜索路径
...
#### 19.2.1  使用 vpath 修改 Makefile 文件
...
### 24.1  使用 include 和多规则更新 Makefile
